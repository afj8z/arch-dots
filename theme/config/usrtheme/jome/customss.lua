...
stylesheet.fg = {
	...
	lnumber = cp.gray._500, -- Line number foreground.
	statusline = cp.gray._50, -- Statusline text.
	whitespace = cp.gray._700, -- Whitespace characters.
}

stylesheet.bg = {
	normal = cp.gray._950, -- Normal (default) window background.
	floating = cp.gray._900, -- Floating window and statusline background.
	...
	visual = cp.ext._visual, -- Visual selection background.
	transparent = cp.transparent,
	dimmed = cp.gray._1000, -- Default background if `dim_inactive_windows` is set.
	sidebar = cp.gray._900,
	border = cp.gray._50,
	highlight = cp.ext._hl,
	black = cp.gray._1000,
	statusline = cp.ext._hl, -- Statusline text.
}

stylesheet.syntax = {
	comment = cp.gray._500, -- Comments.
	conditional = cp.red._400, -- Conditional keywords, e.g., `if`, `when`.
	constant = cp.opal._500, -- Constants, e.g., upper-case variables in Python.
	field = cp.sky._500, -- Object and struct fields.
	func = cp.indigo._400, -- Function definitions.
	identifier = cp.gray._200, -- Identifiers.
	keyword = cp.red._500, -- Keywords.
	number = cp.ext._num, -- Numbers and floats.
	operator = cp.ext._num, -- Operators, e.g., `is`, `not`.
	preprocessor = cp.indigo._400, -- Preprocessor #if, #else, etc.
	punctuation = cp.gray._50, -- Any punctuation, e.g., brackets and commas.
	...
	variable = cp.gray_50, -- Variables.
}

stylesheet.syntax.builtin = {
	const = cp.orange._500, -- Built-in constants, e.g., `nil` in Lua.
	func = cp.indigo._500, -- Built-in functions, e.g., `print` in Lua.
	type = cp.red._500, -- Built-in types, e.g., `i32` in Rust.
	var = cp.sand._500, -- Language-defined variables, e.g., `self` in Python.
}
