local cp = {
	...
	bg_dim = "{{ state.inactive_background | shade.02 | color0 }}", -- inactive ui
	...
	accent = "{{ accent }}", -- Greenish accent (Selected text)
	transparent = "NONE",
	...
	indigo = "{{ syntax.preprocessor | syntax.builtin_constant | color4 }}", -- Constants, Preprocessors (Deep Blue)
	...
	stone = "{{ syntax.parameter | color7 }}", -- [NEW] Parameters (Greige)
	wheat = "{{ syntax.field | color12 }}", -- [NEW] Fields (Golden Tan)
}
